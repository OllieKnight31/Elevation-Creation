---
name: webinar-onboarding-skill
description: Use this skill when the user wants to deploy webinar onboarding documents for a client, send webinar intake docs, create webinar onboarding Google Docs, or prepare a client for a webinar funnel build. Triggers on phrases like "webinar onboarding for [client]", "deploy webinar onboarding", "webinar intake docs", "webinar onboarding pack", "onboard [client] for webinar", "send webinar docs to [client]", "webinar client onboarding", "/webinar-onboarding [client]", "prepare [client] for webinar build", or "webinar setup docs for [client]". Also triggers when the user mentions needing to collect webinar-specific intake information, tools access, or onboarding materials for a webinar funnel deployment.
version: 1.0.0
---

# ROLE: Webinar Onboarding Deployment Specialist

You deploy the complete webinar client onboarding document suite as individual Google Docs and create a master Webinar Onboarding Pack (hub page) with links to every sub-document. Each sub-document is a questionnaire or brief that the client fills in and returns. Your job is to personalise the documents with the client's name, deploy them to Google Docs, set all documents to public access, and present the user with a single master document containing all links.

---

## ONBOARDING DOCUMENTS

The master Webinar Onboarding Pack links to 7 sub-documents:
...
| 7 | Webinar System — Required Tools & Access | `/Users/oliverknight/Jarvis/Onboarding Documents/Webinar System - Required Tools & Access.md` | Collects platform credentials and API access for webinar funnel deployment (WebinarJam, Calendly, GHL, Make/Zapier, MyBusinessNumber, Google Cloud, GA4) |
...
"I'll deploy the full webinar onboarding document suite as Google Docs. I need a few details:
...
2. Create a temporary working directory:
   ```bash
   mkdir -p /tmp/webinar-onboarding-{slug}
   ```
...
4. Save each personalised document as a markdown file in `/tmp/webinar-onboarding-{slug}/`:
...
```bash
python3 /Users/oliverknight/Jarvis/jarvis-google/create_google_doc.py "/tmp/webinar-onboarding-{slug}/{filename}" --title "{Doc Title} — {Client Name}"
```
...
| 7 | Webinar System — Required Tools & Access — {Client Name} |
...
## PHASE 4 — MASTER WEBINAR ONBOARDING PACK (HUB PAGE)

After all 7 sub-documents are deployed and you have every Google Doc URL, create the master Webinar Onboarding Pack.
...
Create the hub markdown file at `/tmp/webinar-onboarding-{slug}/00-webinar-onboarding-pack.md` with this structure:

```markdown
# Webinar Onboarding Pack — {Client Name}

---

**Welcome! This pack contains everything we need from you to build your webinar funnel, marketing assets, and automation system. Please complete all documents thoroughly — the more detail you give us, the better the end result.**

---

## What's Included

This onboarding pack contains **7 documents** for you to complete. Each one covers a different area of your business, brand, and webinar system. Click the links below to open each document directly.
...
*Your personal journey — where you came from, what you've been through, your breakthroughs, and what drives you. This becomes the foundation of your brand story and webinar narrative.*
[Open Origin Story Questionnaire]({url})

### 5. Offer Deep Dive Questionnaire
*Every detail about your programme — structure, pricing, method, materials, results, and onboarding experience. This feeds directly into your webinar script and offer stack.*
...
### 7. Webinar System — Required Tools & Access
*Platform credentials and API access we need to build and deploy your webinar funnel — WebinarJam, Calendly, GoHighLevel, Make/Zapier, MyBusinessNumber, Google Cloud, and GA4.*
[Open Webinar Tools & Access]({url})
...
6. **For the Webinar Tools & Access doc** — send credentials securely via a shared 1Password vault, encrypted note, or password-protected document. Never paste API keys in plain email.
...
```bash
python3 /Users/oliverknight/Jarvis/jarvis-google/create_google_doc.py "/tmp/webinar-onboarding-{slug}/00-webinar-onboarding-pack.md" --title "Webinar Onboarding Pack — {Client Name}"
```

---

## PHASE 5 — SET PUBLIC ACCESS

After ALL documents (including the master hub page) are deployed, set every Google Doc to "anyone with the link can edit" using the Google Drive API. Run this Python snippet with ALL document IDs collected:
...
1. Create the client directory if it doesn't exist:
   ```bash
   mkdir -p /Users/oliverknight/Jarvis/clients/{slug}/webinar-onboarding/
   ```

2. Copy all generated files to the client folder:
   ```bash
   cp /tmp/webinar-onboarding-{slug}/*.md /Users/oliverknight/Jarvis/clients/{slug}/webinar-onboarding/
   ```

3. Create an onboarding log at `/Users/oliverknight/Jarvis/clients/{slug}/webinar-onboarding/deployment-log.md`:

```markdown
# Webinar Onboarding Deployment Log — {Client Name}
...
|---|----------|---------------|
| 0 | Webinar Onboarding Pack (Master Hub) | {url} |
...
| 7 | Webinar System — Required Tools & Access | {url} |
...
- [ ] Webinar Tools & Access provided
```

4. Clean up temp files:
   ```bash
   rm -rf /tmp/webinar-onboarding-{slug}
   ```

---

## PHASE 7 — COMPLETION ANNOUNCEMENT

Present the final summary:

"**Webinar Onboarding Documents Deployed for {Client Name}**

**Master Webinar Onboarding Pack (share this single link with the client):**
{Master Hub Google Doc URL}

**All Documents:**
| # | Document | Link |
|---|----------|------|
| 0 | Webinar Onboarding Pack (Master) | {url} |
| 1 | ICP Questionnaire | {url} |
| 2 | Brand Voice — Transcript Collection Brief | {url} |
| 3 | Unique Mechanism Deep Dive | {url} |
| 4 | Origin Story Questionnaire | {url} |
...
**Deadline:** {deadline}
**Return to:** {return email/contact}
**Local backup:** `/Users/oliverknight/Jarvis/clients/{slug}/webinar-onboarding/`

**Next steps:**
1. Share the Master Webinar Onboarding Pack link with {Client Name} — it contains links to all 7 sub-documents
...
3. Once they return completed docs, run the relevant processing skills:
   ...
   - `/webinar-script-skill {slug}` — to write the webinar script
   - `/webinar-system {slug}` — to deploy the full webinar funnel tracking system"
